        ...
        if event is not None and self._state != EntityState.STUNNED:
            ...
            if event.type == pygame.KEYUP:  # Caso uma tecla seja solta
                ...
                if event.key == pygame.K_s or event.key == pygame.K_w:  # Teclas verticais
                    ...
                    self.direction[1] = 0
        # Redefine as direções caso o jogador esteja artodoado
        elif self._state == EntityState.STUNNED:

            self.direction[0] = 0
            self.direction[1] = 0

        # Faz a contagem do ataque e o redefine caso necessário
        if self._state == EntityState.ATTACKING:
            ...
            if self._attack_time_counter > 60 * self._attack_time:
                ...
                self._attack_time_counter = 0
                self._state = EntityState.IDLING
        # Faz a contagem do atordoamento e o redefine caso necessário
        elif self._state == EntityState.STUNNED:
        ...
        if distance_from_player <= self._sight_distance and \
           ...
           self._state != EntityState.STUNNED:
            ...
            if (player_position[0] - self.position[0]) == 0.0:  # Caso a variação de X seja 0
                ...
                ang_coeff = (self.position[1] - player_position[1]) / \
            ...
            for obstacle in obstacles:  # Loop para cada obstáculo
                ...
                # Caso o jogador esteja na direita do monstro
                if player_position[0] >= self.position[0]:

                    # Se o obstáculo não está antes do monstro ou adiante do jogador
                    if obstacle.rect.right <= self.position[0] or \
                       obstacle.rect.left >= player_position[0]:

                        obstacle_is_in_range_x = False
                else:  # Caso o jogador esteja na esquerda do monstro

                    # Se o obstáculo não está antes do monstro ou adiante do jogador
                    if obstacle.rect.left >= self.position[0] or \
                       obstacle.rect.right <= player_position[0]:

                        obstacle_is_in_range_x = False

                # Caso o jogador esteja acima do monstro
                if -player_position[1] >= -self.position[1]:

                    # Se o obstáculo não está antes do monstro ou adiante do jogador
                    if -obstacle.rect.top <= -self.position[1] or \
                       -obstacle.rect.bottom >= -player_position[1]:

                        obstacle_is_in_range_y = False
                else:  # Caso o jogador esteja abaixo do monstro

                    # Se o obstáculo não está antes do monstro ou adiante do jogador
                    if -obstacle.rect.bottom >= -self.position[1] or \
                       -obstacle.rect.top <= -player_position[1]:
                ...
                if obstacle_is_in_range_x and obstacle_is_in_range_y:
                    ...
                    top_left = (obstacle.rect.topleft[0] - self.position[0],
                                self.position[1] - obstacle.rect.topleft[1])
                    top_right = (obstacle.rect.topright[0] - self.position[0],
                                 self.position[1] - obstacle.rect.topright[1])
                    botton_right = (obstacle.rect.bottomright[0] - self.position[0],
                                    self.position[1] - obstacle.rect.bottomright[1])
                    botton_left = (obstacle.rect.bottomleft[0] - self.position[0],
                                   self.position[1] - obstacle.rect.bottomleft[1])
            ...
            if has_line_of_sight:  # Caso o monstro tenha visão do jogador
                ...
                if abs(player_position[0] - self.position[0]) > 3.0:
                    ...
                    # Se o jogador está na direita do monstro
                    if self.position[0] < player_position[0]:

                        self.direction[0] = 1
                        self.velocity[0] = self.speed
                    # Se o jogador está na esquerda do monstro
                ...
                if abs(player_position[1] - self.position[1]) > 3.0:
                    ...
                    # Se o jogador está acima do monstro
                    if self.position[1] < player_position[1]:

                        self.direction[1] = 1
                        self.velocity[1] = self.speed
                    # Se o jogador está abaixo do monstro
...
class HealthPotion(Entity):
    ...
    def __init__(self, position):
        ...
        self.sprites.add(graphics.HealthPotionBaseSprite(
