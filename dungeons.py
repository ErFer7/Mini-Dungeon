                        ...
                        for char in line:  # Para cada caractere na linha
                            ...
                            if char in '#;':  # Se o caractere é um obstáculo
                                ...
                                if self.doors[int(char)]:  # Caso tenha uma porta nesta posição
                                    ...
                                    self.trigger_sprites.add(
                                        graphics.DoorSprite(position, int(char), char))
                                else:

                                    if char == '0':  # Parede normal

                                        self.collision_sprites.add(
                                            graphics.WallSprite(position, '#'))
                                ...
                                if char != '.':  # Parede de base lateral
                                    ...
                                    self.sprites.add(graphics.WallSprite(
                                        (position[0], position[1] - 24), char))
                                else:  # Parede de base normal

                                    self.sprites.add(graphics.WallSprite(
                                        (position[0], position[1] - 24), '#'))
                                ...
                                # Há 1/110 de chance de gerar uma decoração (caveira)
                        ...
                        for char in line:  # Para cada caractere na linha
                            ...
                            if char == 'P':  # Jogador
                                ...
                                if self.doors[int(char)]:
                                    ...
                                    self.doors_leaving_position[int(char)] = [
                                ...
                                if randint(0, 10) <= difficulty or initial_monster_ammount == 0:
                                    ...
                                    self.entities[f"Monster_{self.monster_ammount}"] = \
                                        entities.Monster([position[0] + 16, position[1] + 16])
...
def generate_dungeon(screen_size, width=1, heigth=1):
    ...
    while True:
        ...
        # Caso todas as posições sejam processadas termina o loop
        ...
        if position[1] > 0 and                                              \
           ([position[0], position[1] - 1] not in room_struct_list) and not \
           (structure[position[1]][position[0]][0] or structure[position[1] - 1][position[0]][1]):

            directions.append(Direction.UP)

        # Caso seja possível ir para baixo
        if position[1] < heigth - 1 and                                     \
           ([position[0], position[1] + 1] not in room_struct_list) and not \
           (structure[position[1]][position[0]][1] or structure[position[1] + 1][position[0]][0]):

            directions.append(Direction.DOWN)

        # Caso seja possível ir para a direita
        if position[0] < width - 1 and                                      \
           ([position[0] + 1, position[1]] not in room_struct_list) and not \
           (structure[position[1]][position[0]][2] or structure[position[1]][position[0] + 1][3]):
        ...
        if position[0] > 0 and                                              \
           ([position[0] - 1, position[1]] not in room_struct_list) and not \
           (structure[position[1]][position[0]][3] or structure[position[1]][position[0] - 1][2]):
    ...
    for i in range(heigth):
        ...
        for j in range(width):
            ...
            room = Room(structure[i][j], choice(os.listdir("Rooms")), screen_size,
                        abs(i - player_ln) + abs(j - player_col), initial_monster_ammount)
